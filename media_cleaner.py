...
def get_brand():
    ...
    print('0:emby\n1:jellyfin')
    brand = input(
        'Enter number for server branding (default ' + defaultbrand + '): ')
    if (brand == ''):
        return (defaultbrand)
    elif (brand == '0'):
        return (defaultbrand)
...
def delete_item(itemID):
    url = url = cfg.server_url + '/Items/' + \
        itemID + '?api_key=' + cfg.access_token
...
def get_auth_key(server_url, username, password, password_sha1):
    ...
    headers = {'X-Emby-Authorization': 'Emby UserId="' + username +
               '", Client="media_cleaner", Device="media_cleaner", DeviceId="media_cleaner", Version="0.2", Token=""', 'Content-Type': 'application/json'}

    req = request.Request(url=server_url + '/Users/AuthenticateByName',
...
def get_isfav_season_series(server_url, user_key, itemId, auth_key):
    ...
    url = server_url + '/Users/' + user_key + \
        '/Items/' + itemId + '?api_key=' + auth_key
...
def get_items(server_url, user_key, auth_key):
    ...
    url = server_url + '/Users/' + user_key + \
        '/Items?Recursive=true&IsPlayed=true&SortBy=Type,SeriesName,ParentIndexNumber,IndexNumber,Name&SortOrder=Ascending&api_key=' + auth_key
    ...
    cut_off_date_movie = datetime.now(
        timezone.utc) - timedelta(cfg.not_played_age_movie)
    cut_off_date_episode = datetime.now(
        timezone.utc) - timedelta(cfg.not_played_age_episode)
    cut_off_date_video = datetime.now(
        timezone.utc) - timedelta(cfg.not_played_age_video)
    cut_off_date_trailer = datetime.now(
        timezone.utc) - timedelta(cfg.not_played_age_trailer)
    deleteItems = []
    ...
    for item in data['Items']:
        ...
        if (item['Type'] == 'Movie'):
            if (
               ...
               (not bool(cfg.keep_favorites_movie)
                ...
                try:
                    item_details = '  ' + item['Type'] + ' - ' + item['Name'] + ' - ' + get_days_since_watched(
                        item['UserData']['LastPlayedDate']) + ' - Favorite: ' + str(item['UserData']['IsFavorite']) + ' - ' + 'MovieID: ' + item['Id']
                except (KeyError):
                    item_details = '  ' + \
                        item['Type'] + ' - ' + \
                        item['Name'] + ' - ' + item['Id']
        ...
        # find tv-episode media items ready to delete
            ...
            if (
               ...
               (not bool(cfg.keep_favorites_episode) or (not itemIsFav))
                ...
                try:
                    item_details = item['Type'] + ' - ' + item['SeriesName'] + ' - ' + get_season_episode(item['ParentIndexNumber'], item['IndexNumber']) + ' - ' + item['Name'] + ' - ' + get_days_since_watched(
                        item['UserData']['LastPlayedDate']) + ' - Favorite: ' + str(itemIsFav) + ' - ' + 'EpisodeID: ' + item['Id']
                except (KeyError):
                    item_details = '  ' + \
                        item['Type'] + ' - ' + \
                        item['Name'] + ' - ' + item['Id']
        ...
        # find video media items ready to delete
            ...
            if (
               ...
               (not bool(cfg.keep_favorites_video)
                ...
                try:
                    item_details = item['Type'] + ' - ' + item['Name'] + ' - ' + get_days_since_watched(
                        item['UserData']['LastPlayedDate']) + ' -  Favorite: ' + str(item['UserData']['IsFavorite']) + ' - ' + 'VideoID: ' + item['Id']
                except (KeyError):
                    item_details = '  ' + \
                        item['Type'] + ' - ' + \
                        item['Name'] + ' - ' + item['Id']
        ...
        # find trailer media items ready to delete
            ...
            if (
               ...
               (not bool(cfg.keep_favorites_trailer)
                ...
                try:
                    item_details = item['Type'] + ' - ' + item['Name'] + ' - ' + get_days_since_watched(
                        item['UserData']['LastPlayedDate']) + ' -  Favorite: ' + str(item['UserData']['IsFavorite']) + ' - ' + 'TrailerID: ' + item['Id']
                except (KeyError):
                    item_details = '  ' + \
                        item['Type'] + ' - ' + \
                        item['Name'] + ' - ' + item['Id']
        ...
        # idk what these are; keep them
            ...
            try:
                item_details = item['Type'] + ' - ' + item['Name'] + ' - Favorite: ' + str(
                    item['UserData']['IsFavorite']) + ' - ID: ' + item['Id']
            except (KeyError):
                item_details = '  ' + item['Type'] + \
                    ' - ' + item['Name'] + ' - ' + item['Id']
                if bool(cfg.DEBUG):
                    # DEBUG
                    print(
    ...
    if len(deleteItems) > 0:
        for item in deleteItems:
            if item['Type'] == 'Movie':
                ...
                try:
                    item_details = item['Type'] + ' - ' + item['SeriesName'] + ' - ' + get_season_episode(
                        item['ParentIndexNumber'], item['IndexNumber']) + ' - ' + item['Name'] + ' - ' + item['Id']
                except (KeyError):
                    item_details = '  ' + \
                        item['Type'] + ' - ' + \
                        item['Name'] + ' - ' + item['Id']
    ...
    if (
        ...
        if (
            ...
            auth_key = get_auth_key(
                server_url, username, password, password_sha1)
            user_key = list_users(server_url, auth_key)
            print('-----------------------------------------------------------')
